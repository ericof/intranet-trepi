...
PROJECT_NAME=intranet-trepi
STACK_NAME=intranet-tre-pi-jus-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://intranet-trepi.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t ericof/intranet-trepi-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t ericof/intranet-trepi-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name intranet-trepi-frontend-acceptance --link intranet-trepi-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ericof/intranet-trepi-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name intranet-trepi-backend-acceptance -d ericof/intranet-trepi-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop intranet-trepi-frontend-acceptance
	@docker stop intranet-trepi-backend-acceptance
